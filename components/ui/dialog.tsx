...
>(({ className, lightOverlay, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ...
    className={cn(
      ...
      lightOverlay ? "bg-white/80" : "bg-black/80",
      className,
    )}
    {...props}
  />
...
>(({ className, children, lightOverlay = false, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      ...
      {!lightOverlay && (
        <DialogPrimitive.Close className="absolute right-[-20px] top-[-20px] rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground text-white">
          <X className="h-6 w-6" />
